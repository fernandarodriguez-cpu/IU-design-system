...
const changelog: VersionEntry[] = [
  ...
  },
  {
    ...
    summary: 'Separación de datos y UI en registry, auto-generación de conteos de componentes, CSS block desde khorStaticTokens, YAML front matter en guías modulares.',
    highlights: [
      'Registry types: AtomData, MoleculeData, OrganismData sin JSX en src/app/registry/',
      'Data-only exports desde AtomsPage, MoleculesPage, OrganismsPage (destructuring omite preview/playground/stateShowcase)',
      'AIExportPage importa solo datos — el generador de markdown ya no arrastra componentes React',
      'scripts/generate-counts.mjs auto-genera khor-counts.json en cada build',
      'generateCssBlock() en khor-theme.ts reemplaza CSS hardcodeado de 200+ líneas',
      'YAML front matter (system, module, dependencies, context_rule) en cada guía individual',
      'Labels dinámicos: conteos de componentes se computan en runtime',
      'KHOR_VERSION centralizado: AppShell, AIExportPage importan desde src/app/version/version.ts',
    ],
    changes: [
      { type: 'added', description: 'Registry types AtomData, MoleculeData, OrganismData en src/app/registry/registry-types.ts.' },
      { type: 'added', description: 'scripts/generate-counts.mjs — genera khor-counts.json desde Object.keys() en cada npm run build.' },
      { type: 'added', description: 'generateCssBlock() en khor-theme.ts — reemplaza 200+ líneas de CSS hardcodeado en AIExportPage. Acepta theme opcional para guías dinámicas al tema.' },
      { type: 'added', description: 'YAML front matter en guías modulares (system, module, dependencies, context_rule) para auto-documentación de contexto LLM.' },
      { type: 'changed', description: 'AtomsPage/MoleculesPage/OrganismsPage exportan atomsData/moleculesData/organismsData (solo datos, sin JSX).' },
      { type: 'changed', description: 'AIExportPage importa desde data-only exports — elimina dependencia de JSX en markdown.' },
      { type: 'changed', description: 'Labels de secciones (Átomos (N), Moléculas (N)) ahora dinámicos desde Object.keys().' },
      { type: 'changed', description: 'AppShell badge actualizado a KHOR_VERSION desde version.ts.' },
    ],
    stats: { added: 4, changed: 4, fixed: 0 },
  },
  {
    version: '6.1.0',
    date: '11 de Junio, 2026',
    codename: 'Multi-Guía LLM & Architecture Hardening',
    summary: 'Descarga de guías individuales por capa, ZIP completo, índice de componentes, version centralizada, fix de KButton danger.',
    highlights: [
      'Multi-guía para LLMs: descarga individual de tokens, átomos, moléculas, organismos como .md',
      'Descarga ZIP completa con JSZip empaquetando las 5 guías',
      'Índice de componentes con tabla compacta de los 81 componentes al inicio del markdown',
      'KHOR_VERSION movido a src/app/version/version.ts como fuente única',
      'Fix KButton danger prop: preview unificado a variant="danger"',
      'Verificación de integridad: build de producción sin errores',
    ],
    changes: [
      { type: 'added', description: 'Descarga individual de guías .md por capa (tokens, atoms, molecules, organisms).' },
      { type: 'added', description: 'Descarga ZIP de todas las guías con JSZip.' },
      { type: 'added', description: 'Índice de componentes con tabla compacta en markdown.' },
      { type: 'added', description: 'src/app/version/version.ts como fuente única de KHOR_VERSION.' },
      { type: 'changed', description: 'KButton preview unificado a variant="danger".' },
    ],
    stats: { added: 4, changed: 1, fixed: 1 },
  },
  {
    version: '6.0.0',
    date: '11 de Junio, 2026',
    codename: 'Khor Audit & CSS/JSON Sync',
    summary: 'Corrección de bugs de renderizado en markdown, sincronización CSS vs khorTokens, accesibilidad, y eliminación de secciones duplicadas.',
    highlights: [
      'JSON tokens corregidos: template literals reemplazados por JSON real en markdown',
      'CSS :root expandido con +90 variables para igualar khorTokens',
      'Radius sincronizados con khorStaticTokens (sm: 6px, md: 8px, lg: 10px, xl: 14px)',
      'KButton A11y corregido: score 100→96, contraste AAA→AA (4.8:1 real)',
      'Secciones duplicadas eliminadas (i18n, Fluid Typography, Reduced Motion, Gobernanza)',
      'KInput.Search/KInput.Password marcados como compound sub-components via aiNotes',
    ],
    changes: [
      { type: 'fixed', description: 'JSON tokens rendering: escape de $ eliminado en template literals.' },
      { type: 'fixed', description: 'CSS :root sincronizado con khorTokens (+90 variables faltantes).' },
      { type: 'fixed', description: 'Radius values corregidos: sm=6px, md=8px, lg=10px, xl=14px.' },
      { type: 'fixed', description: 'KButton A11y: score real 96, contraste AA 4.8:1.' },
      { type: 'changed', description: 'Secciones duplicadas eliminadas del markdown generado.' },
    ],
    stats: { added: 0, changed: 1, fixed: 4 },
  },
  {
    version: '5.3.0-alpha',
