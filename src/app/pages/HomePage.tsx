/**
 * ╔═══════════════════════════════════════════╗
 * ║  KHOR DESIGN SYSTEM — v5.0.0-alpha        ║
...
export function HomePage() {
  ...
  const handleDownloadKDS = async () => {
    ...
    ds.file('README.md', `# Khor Design System v5.0-alpha\n...`);
    ds.file('KHOR_AI_GUIDE.md', generateMarkdown(defaultSections, themeConfig));
    const blob = await zip.generateAsync({ type: 'blob' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `khor-ds-v5.0-alpha.zip`;
    a.click();
    kToast({ type: 'success', title: 'Exportación Exitosa', description: 'Paquete v5.0-alpha listo.' });
  };
      ...
      <div className="relative overflow-hidden rounded-[3rem] bg-khor-secondary text-white p-16 lg:p-24 mb-20 shadow-2xl border border-white/10 group">
        ...
        <div className="relative z-10">
          <div className="flex flex-wrap gap-3 mb-10">
            <KTag color="volcano" className="border-white/20 text-white bg-white/10 px-4 py-1 backdrop-blur-xl">v5.0.0-alpha</KTag>
