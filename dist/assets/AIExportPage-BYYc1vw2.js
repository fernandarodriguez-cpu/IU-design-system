...
:root {
  ...
  --khor-chart-primary: ${o.primary};   --khor-chart-secondary: ${o.secondary};
  --khor-chart-accent: ${o.accent};    --khor-chart-success: ${o.success};
  --khor-chart-error: ${o.error};     --khor-chart-info: ${o.info};
  ...
  --khor-form-error-bg: ${o.error}15;   --khor-form-error-border: ${o.error};   --khor-form-error-text: ${o.error};
  --khor-form-success-bg: ${o.success}15; --khor-form-success-border: ${o.success}; --khor-form-success-text: ${o.success};
  --khor-form-warning-bg: ${o.warning}15; --khor-form-warning-border: ${o.warning}; --khor-form-warning-text: ${o.warning};
  --khor-form-focus-ring: ${o.primary};

  /* Semantic Layer 2: Actions */
  --khor-action-primary-default: ${o.primary}; --khor-action-primary-hover: #e8644f;
  --khor-action-secondary-default: ${o.secondary}; --khor-action-secondary-hover: #0a2270;
  --khor-action-danger-default: ${o.error}; --khor-action-danger-hover: #B71C1C;
  ...
  --khor-surface-selected: ${o.primary}15; --khor-surface-subtle: #F4F6F8;
  ...
  --khor-border-strong: #A0AEC0; --khor-border-focus: ${o.primary};
  --khor-border-error: ${o.error}; --khor-border-disabled: #EDF0F1;

  /* Semantic Layer 2: Typography */
  --khor-text-primary: ${o.secondary}; --khor-text-secondary: #475a8f;
...
| Token | Curva | Uso |
|-------|-------|-----|
| \`--khor-ease-standard\` | cubic-bezier(0.4, 0, 0.2, 1) | Movimiento general, cambios de estado |
| \`--khor-ease-decelerate\` | cubic-bezier(0, 0, 0.2, 1) | Elementos que entran a escena |
| \`--khor-ease-accelerate\` | cubic-bezier(0.4, 0, 1, 1) | Elementos que salen de escena |
| \`--khor-ease-spring\` | cubic-bezier(0.175, 0.885, 0.32, 1.275) | Botones, toggles, microinteracciones |
| \`--khor-ease-bounce\` | cubic-bezier(0.34, 1.56, 0.64, 1) | Badges, contadores, notificaciones |
| \`--khor-ease-smooth\` | cubic-bezier(0.45, 0, 0.55, 1) | Scroll suave, transiciones de página |

**Reglas para la IA:**
1. **Entrada vs Salida:** Los elementos que entran deben usar \`duration-entrance\` + \`ease-decelerate\`. Los que salen deben usar \`duration-exit\` + \`ease-accelerate\`. La salida SIEMPRE es más rápida que la entrada.
...
|---------|-------|---------|
| **v5.0.0-beta** | ${g} | **Enterprise Hardening:** Layer 3 Component Tokens (KButton, KInput, KCard), RTL/i18n native con CSS Logical Properties, SSR guidelines (Next.js/Remix), Security policies (XSS/CSP), Testing strategy (Playwright+Axe), SemVer governance y AI Agent contracts. |
| **v5.0.0-alpha** | ${g} | **The World-Class Foundation:** Migración total a arquitectura W3C Design Tokens, tipografía fluida, KDataGrid empresarial, Command Palette con acciones, KFormWizard y testing con Playwright. |
| **v4.4.1** | ${g} | **KQA God Mode:** Sincronización de más de 20 organismos y moléculas con estados explícitos y Layer 3 Contextual Tokens. Nuevo script de auditoría y Patrones Maestros. |
...
---`),d.has("darkmode")&&s.push("\n## Dark Mode\n\nEl sistema soporta modo oscuro via clase `.dark` en `<html>`. Se activa con `useTheme()` del `ThemeProvider`.\n\n### Tokens Oscuros (principales cambios)\n\n| Token | Light | Dark |\n|-------|-------|------|\n| neutral-50 | `#FFFFFF` | `#1A1B2E` |\n| neutral-100 | `#EDF0F1` | `#22243A` |\n| neutral-200 | `#D5DBE0` | `#2E3148` |\n| neutral-300 | `#A0AEC0` | `#4A4E6A` |\n| neutral-400 | `#718096` | `#8B90A8` |\n| neutral-500 | `#4A5568` | `#B0B4C8` |\n| neutral-600 | `#5A6475` | `#9BA3B5` |\n| neutral-700 | `#3D4552` | `#B8BDC8` |\n| neutral-800 | `#252C38` | `#D0D3DA` |\n| neutral-900 | `#000000` | `#E8EAF0` |\n| accent | `#FF9500` | `#FFB340` |\n| navy | `#051758` | `#8BA3D9` |\n| success | `#2E7D32` | `#4CAF50` |\n| error | `#D32F2F` | `#EF5350` |\n\n### Uso del ThemeProvider\n\n```tsx\nimport { ThemeProvider, useTheme } from './theme/theme-context';\n\n// En el root:\n<ThemeProvider>\n  <App />\n</ThemeProvider>\n\n// En cualquier componente:\nconst { mode, toggle, isDark } = useTheme();\n// mode: 'light' | 'dark'\n// toggle(): cambia el modo\n// isDark: boolean\n```\n\n### Recomendacion para componentes custom\n\nUsa `var(--khor-*)` en vez de valores hardcoded para que respondan al cambio de tema:\n```tsx\n// MAL:\nstyle={{ backgroundColor: '#FFFFFF', color: '#000000' }}\n\n// BIEN:\nstyle={{ backgroundColor: 'var(--card)', color: 'var(--foreground)' }}\n```\n\n---");const u=(l,b,y,k)=>{const c=Object.keys(k);let n=`## ${l} (${c.length} componentes)

${b}
Importar desde: \`${y}\`

`;return c.forEach(C=>{const r=k[C];n+=`### ${r.name}
${r.description}

`,r.aiNotes&&(n+=`> **Directrices IA**: ${r.aiNotes}

`),r.a11ySummary&&(n+=`**Accesibilidad (ARIA & Keyboard - Score: ${r.a11ySummary.score}/100)**
`,n+=`- **Keyboard:** ${r.a11ySummary.keyboard.join(" ")}
`,n+=`- **ARIA:** ${r.a11ySummary.aria.join(" ")}
`,n+=`- **Contraste:** ${r.a11ySummary.contrast}

`),r.props&&r.props.length>0&&(n+=`**Props Principales:**
`,n+=`| Prop | Tipo | Requerido | Default | Descripcion |
`,n+=`|------|------|-----------|---------|-------------|
`,r.props.forEach(p=>{n+=`| \`${p.name}\` | \`${p.type}\` | ${p.required?"Si":"No"} | ${p.default?`\`${p.default}\``:"-"} | ${p.description} |
`}),n+=`
`),r.code&&(n+=`**Ejemplo de Uso:**
\`\`\`tsx
${r.code}
\`\`\`

`),r.guidelines&&r.guidelines.length>0&&(n+=`**Guidelines UX:**
`,r.guidelines.forEach(p=>{n+=`- ${p}
`}),n+=`
`),n+=`---

`}),n};return d.has("atoms")&&s.push(u("Átomos","Unidades indivisibles y fundamentales.","import { KButton } from '@khor/design-system/atoms/index'",z)),d.has("molecules")&&s.push(u("Moléculas","Combinaciones de átomos con lógica de forma reutilizable.","import { KFormField } from '@khor/design-system/molecules/index'",M)),d.has("organisms")&&s.push(u("Organismos","Componentes complejos o Layouts masivos con lógicas de portal, focus-traps y alto consumo de hooks.","import { KDataTable } from '@khor/design-system/organisms/index'",U)),d.has("templates")&&s.push(`
...
`),B.forEach(l=>{s.push(`
### Patrón Modular: ${l.title}
${l.description}
\`\`\`tsx
${l.code}
\`\`\`
`)}),s.push("---")),d.has("examples")&&s.push(`
## Ejemplos de Codigo
...
---
`),s.join(`
`)}function Y(f){return f.split(/\s+/).filter(Boolean).length}function ue(){const{themeConfig:f}=P(),[o,d]=h.useState(Z),[s,g]=h.useState(!1),[u,l]=h.useState(!0),[b,y]=h.useState(!1),k=h.useRef(null),c=h.useMemo(()=>Q(o,f),[o,f]),n=h.useMemo(()=>Y(c),[c]),C=h.useMemo(()=>c.split(`
`).length,[c]),r=o.filter(a=>a.enabled).length,p=a=>{d(t=>t.map(m=>m.id===a?{...m,enabled:!m.enabled}:m))},T=()=>{const a=new Blob([c],{type:"text/markdown;charset=utf-8"}),t=URL.createObjectURL(a),m=document.createElement("a");m.href=t,m.download="khor-design-system-ai-guide.md",document.body.appendChild(m),m.click(),document.body.removeChild(m),URL.revokeObjectURL(t),v({type:"success",title:"Descarga iniciada",description:"khor-design-system-ai-guide.md descargado."})},S=async()=>{let a=!1;try{await navigator.clipboard.writeText(c),a=!0}catch{try{const t=document.createElement("textarea");t.value=c,t.style.position="fixed",t.style.left="-9999px",t.style.top="-9999px",t.style.opacity="0",document.body.appendChild(t),t.focus(),t.select(),a=document.execCommand("copy"),document.body.removeChild(t)}catch{a=!1}}a?(g(!0),v({type:"success",title:"Copiado",description:"Todo el markdown fue copiado al portapapeles."}),setTimeout(()=>g(!1),2e3)):(v({type:"warning",title:"No se pudo copiar automaticamente",description:'Usa el boton "Seleccionar todo" en la vista previa y copia manualmente con Ctrl+C / Cmd+C.'}),l(!0),y(!0))},E=()=>{if(k.current){const a=document.createRange();a.selectNodeContents(k.current);const t=window.getSelection();t&&(t.removeAllRanges(),t.addRange(a)),v({type:"info",title:"Texto seleccionado",description:"Ahora presiona Ctrl+C (o Cmd+C) para copiar."})}},D=()=>d(a=>a.map(t=>({...t,enabled:!0}))),I=()=>d(a=>a.map(t=>({...t,enabled:!1})));return e.jsxs("div",{style:{fontFamily:i.typography.fontPrimary},children:[e.jsx("div",{style:{marginBottom:32},children:e.jsxs("div",{style:{display:"flex",alignItems:"center",gap:12,marginBottom:8},children:[e.jsx("div",{style:{width:44,height:44,borderRadius:i.radius.md,background:"linear-gradient(135deg, #8B5CF6 0%, #EC4899 100%)",display:"flex",alignItems:"center",justifyContent:"center"},children:e.jsx(N,{size:24,color:"#fff"})}),e.jsxs("div",{children:[e.jsx("h1",{style:{margin:0,fontSize:28,color:"var(--foreground)"},children:"Exportar Guia para IA"}),e.jsx("p",{style:{margin:0,fontSize:14,color:"var(--muted-foreground)"},children:"Genera un archivo .md con todo el sistema de diseno para usar como contexto en cualquier IA."})]})]})}),e.jsx(L,{type:"info",title:"Prompt portatil para IAs generativas",description:"Este archivo .md contiene tokens, APIs de componentes, patrones y ejemplos del sistema Khor. Pegalo como contexto en ChatGPT, Claude, Cursor, Figma Make, v0 o cualquier asistente IA para que genere interfaces 100% consistentes con Khor.",className:"mb-6"}),e.jsxs("div",{style:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:24},children:[e.jsxs("div",{style:{backgroundColor:"var(--card)",borderRadius:i.radius.lg,padding:24,border:"1px solid var(--border)"},children:[e.jsxs("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16},children:[e.jsxs("h3",{style:{margin:0,fontSize:16,color:"var(--foreground)"},children:[e.jsx(O,{size:18,style:{display:"inline",marginRight:8,verticalAlign:"middle"}}),"Secciones a incluir"]}),e.jsxs("div",{style:{display:"flex",gap:8},children:[e.jsx("button",{onClick:D,style:{background:"none",border:"none",cursor:"pointer",fontSize:12,color:i.colors.brand.primary,fontFamily:i.typography.fontPrimary,textDecoration:"underline"},children:"Todas"}),e.jsx("button",{onClick:I,style:{background:"none",border:"none",cursor:"pointer",fontSize:12,color:"var(--muted-foreground)",fontFamily:i.typography.fontPrimary,textDecoration:"underline"},children:"Ninguna"})]})]}),e.jsx("div",{style:{display:"flex",flexDirection:"column",gap:8},children:o.map(a=>e.jsxs("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 12px",borderRadius:i.radius.md,border:`1px solid ${a.enabled?"var(--khor-primary)30":"var(--border)"}`,backgroundColor:a.enabled?"rgba(224,77,54,0.03)":"transparent",transition:"all 0.15s ease"},children:[e.jsxs("div",{style:{flex:1,minWidth:0},children:[e.jsx("div",{style:{fontSize:14,fontWeight:500,color:"var(--foreground)"},children:a.label}),e.jsx("div",{style:{fontSize:12,color:"var(--muted-foreground)",marginTop:2},children:a.description})]}),e.jsx(R,{checked:a.enabled,onCheckedChange:()=>p(a.id),size:"small"})]},a.id))})]}),e.jsxs("div",{style:{display:"flex",flexDirection:"column",gap:16},children:[e.jsxs("div",{style:{backgroundColor:"var(--card)",borderRadius:i.radius.lg,padding:24,border:"1px solid var(--border)"},children:[e.jsxs("h3",{style:{margin:"0 0 16px",fontSize:16,color:"var(--foreground)"},children:[e.jsx(K,{size:18,style:{display:"inline",marginRight:8,verticalAlign:"middle"}}),"Resumen del documento"]}),e.jsx("div",{style:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12},children:[{label:"Secciones",value:`${r}/${o.length}`,color:i.colors.brand.primary},{label:"Palabras",value:n.toLocaleString(),color:i.colors.brand.navy},{label:"Lineas",value:C.toLocaleString(),color:i.colors.brand.accent},{label:"Tamano aprox.",value:`${(new Blob([c]).size/1024).toFixed(1)} KB`,color:i.colors.feedback.success}].map(a=>e.jsxs("div",{style:{padding:12,borderRadius:i.radius.md,backgroundColor:`${a.color}08`,border:`1px solid ${a.color}15`},children:[e.jsx("div",{style:{fontSize:12,color:"var(--muted-foreground)",marginBottom:4},children:a.label}),e.jsx("div",{style:{fontSize:20,fontWeight:700,color:a.color},children:a.value})]},a.label))})]}),e.jsxs("div",{style:{backgroundColor:"var(--card)",borderRadius:i.radius.lg,padding:24,border:"1px solid var(--border)"},children:[e.jsxs("h3",{style:{margin:"0 0 12px",fontSize:16,color:"var(--foreground)"},children:[e.jsx($,{size:18,style:{display:"inline",marginRight:8,verticalAlign:"middle"}}),"Compatible con"]}),e.jsx("div",{style:{display:"flex",flexWrap:"wrap",gap:8},children:["ChatGPT","Claude","Figma Make","Cursor","v0 (Vercel)","Windsurf","GitHub Copilot","Gemini","Bolt"].map(a=>e.jsx("span",{style:{padding:"4px 12px",borderRadius:999,fontSize:12,fontWeight:500,backgroundColor:"var(--khor-neutral-100)",color:"var(--foreground)",border:"1px solid var(--border)"},children:a},a))})]}),e.jsxs("div",{style:{backgroundColor:"var(--card)",borderRadius:i.radius.lg,padding:24,border:"1px solid var(--border)"},children:[e.jsxs("h3",{style:{margin:"0 0 16px",fontSize:16,color:"var(--foreground)"},children:[e.jsx(q,{size:18,style:{display:"inline",marginRight:8,verticalAlign:"middle"}}),"Acciones"]}),e.jsxs("div",{style:{display:"flex",flexDirection:"column",gap:10},children:[e.jsx(x,{variant:"primary",block:!0,icon:e.jsx(G,{size:16}),onClick:T,disabled:r===0,children:"Descargar .md"}),e.jsx(x,{variant:"outline",block:!0,icon:s?e.jsx(A,{size:16}):e.jsx(F,{size:16}),onClick:S,disabled:r===0,children:s?"Copiado!":"Copiar al portapapeles"}),e.jsx(x,{variant:"ghost",block:!0,icon:u?e.jsx(V,{size:16}):e.jsx(H,{size:16}),onClick:()=>l(!u),disabled:r===0,children:u?"Ocultar vista previa":"Ver vista previa"})]})]})]})]}),u&&e.jsxs("div",{style:{backgroundColor:"var(--card)",borderRadius:i.radius.lg,border:"1px solid var(--border)",overflow:"hidden"},children:[e.jsxs("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 20px",borderBottom:"1px solid var(--border)",backgroundColor:"var(--khor-neutral-100)"},children:[e.jsxs("div",{style:{display:"flex",alignItems:"center",gap:8},children:[e.jsx(K,{size:16,style:{color:"var(--muted-foreground)"}}),e.jsx("span",{style:{fontSize:13,fontWeight:500,color:"var(--foreground)"},children:"khor-design-system-ai-guide.md"}),e.jsx(j,{status:"info",label:`${n.toLocaleString()} palabras`,dot:!1})]}),e.jsxs("button",{onClick:()=>y(!b),style:{background:"none",border:"none",cursor:"pointer",display:"flex",alignItems:"center",gap:4,fontSize:12,color:"var(--muted-foreground)",fontFamily:i.typography.fontPrimary},children:[b?e.jsx(W,{size:14}):e.jsx(_,{size:14}),b?"Colapsar":"Expandir completo"]})]}),e.jsx("pre",{ref:k,style:{padding:20,margin:0,fontSize:12,lineHeight:1.6,color:"var(--foreground)",fontFamily:"'Plus Jakarta Sans', monospace",whiteSpace:"pre-wrap",wordBreak:"break-word",maxHeight:b?"none":500,overflow:b?"visible":"auto"},children:c}),e.jsxs("div",{style:{padding:"12px 20px",borderTop:"1px solid var(--border)",display:"flex",gap:8},children:[e.jsx(x,{variant:"outline",icon:s?e.jsx(A,{size:16}):e.jsx(F,{size:16}),onClick:S,style:{flex:1},children:s?"Copiado!":"Copiar al portapapeles"}),e.jsx(x,{variant:"ghost",icon:e.jsx(X,{size:16}),onClick:E,style:{flex:1},children:"Seleccionar todo (Ctrl+C)"})]})]})]})}export{ue as AIExportPage,J as KHOR_VERSION,Z as defaultSections,Q as generateMarkdown};
